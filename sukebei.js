...
async function performSearch(query, options = {}) {
  ...
  const fetchFunc = query.fetch;
  
  if (!fetchFunc) {
    throw new Error("Fetch function not provided by Hayase");
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to connect to Sukebei Nyaa (HTTP ${response.status})`);
    }
    ...
    throw new Error(`Sukebei Nyaa search failed: ${error.message}`);
  }
}
...
export default {
  async test() {
    try {
      ...
      if (!resp.ok) throw new Error(`Cannot reach Sukebei Nyaa (HTTP ${resp.status})`);
      return true;
    } catch (error) {
      throw new Error(`Sukebei Nyaa extension test failed: ${error.message}`);
    }
  },
